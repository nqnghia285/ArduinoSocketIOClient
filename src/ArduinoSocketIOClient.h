...
class ArduinoSocketIOClient : protected WebSocketsClient {
 public:
   ...
   void begin(const char *host, uint16_t port = DEFAULT_PORT,
              const char *nsp = DEFAULT_PATH, const char *url = DEFAULT_URL,
              const char *protocol = DEFAULT_PROTOCOL);
   void begin(String host, uint16_t port = DEFAULT_PORT,
              String nsp = DEFAULT_PATH, String url = DEFAULT_URL,
              String protocol = DEFAULT_PROTOCOL);

#ifdef HAS_SSL
   void beginSSL(const char *host, const char *nsp = DEFAULT_PATH,
                 uint16_t port = DEFAULT_SSL_PORT,
                 const char *url = DEFAULT_URL,
                 const char *protocol = DEFAULT_PROTOCOL);
   void beginSSL(String host, String nsp = DEFAULT_PATH,
                 uint16_t port = DEFAULT_SSL_PORT, String url = DEFAULT_URL,
                 String protocol = DEFAULT_PROTOCOL);
#ifndef SSL_AXTLS
   void beginSSLWithCA(const char *host, const char *nsp = DEFAULT_PATH,
                       uint16_t port = DEFAULT_SSL_PORT,
                       const char *url = DEFAULT_URL,
   ...
   bool send(socketIOmessageType_t type, char *payload, size_t length = 0,
             bool headerToPayload = false);
   bool send(socketIOmessageType_t type, const char *payload,
             size_t length = 0);
   ...
   void on(const char *event,
           std::function<void(const char *payload, size_t length)>);
 ...
 protected:
   ...
   std::map<String, std::function<void(const char *payload, size_t length)>>
       _events;
   ...
   void socketEvent(socketIOmessageType_t type, uint8_t *payload,
                    size_t length);
